...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        epoch_avg_train_accuracy = torch.mean(torch.concat(metrics['train_acc']))
        logger.add_scalar('train/accuracy', epoch_avg_train_accuracy, global_step)
        ...
        epoch_avg_val_acc = torch.mean(torch.concat(metrics['val_acc']))
        logger.add_scalar('train/validation', epoch_avg_val_acc, global_step)
